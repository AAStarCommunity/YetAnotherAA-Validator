...
- **🚀 部署你自己的测试网 DVT（always-on，任何人可复制）** →
  [`deploy/README.md`](deploy/README.md) —— clone + 配置即可起一个公网 always-on
  DVT（Docker + Cloudflare named
  tunnel；含生成独立密钥、链上注册、起服务、验证）。三套部署方案对比见
  [`deploy/DEPLOYMENT_OPTIONS.md`](deploy/DEPLOYMENT_OPTIONS.md)；无许可 staking 接入（建自己的社区 DVT）见
  `deploy/README.md` §3(b)。AAStar 官方参考节点：`https://dvt1.aastar.io` ·
  `https://dvt2.aastar.io` · `https://dvt3.aastar.io`（SDK 默认配置
...
- **全局版本路线图(后续开发 follow 此)** → [`docs/ROADMAP.md`](docs/ROADMAP.md)
- **跨仓库协调 hub** →
