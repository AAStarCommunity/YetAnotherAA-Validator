...
| 仓库                                    | 关系                   | 钉定基线         | 集成点                                                                                                                                                  |
| --------------------------------------- | ---------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **AAStarCommunity/SuperPaymaster**      | UPSTREAM               | `v5.4.0-beta.1`  | PolicyRegistry `0x37e4E40e69Fb7d5C3fbAA0F52A4002D27472Ff29`（layer-1 `checkPolicy`）+ ROLE_DVT 注册 + BLSAggregator                                     |
| **AAStarCommunity/airaccount-contract** | 协作（链上验签消费方） | `v0.18.0-beta.2` | `AAStarBLSAlgorithm` `0xA9EE4f8A59fCE1B56f9da8e153c3f5F38D3C59ED`：`validate(userOpHash, [nodeIds][blsSig])`，DST `_POP_`，messagePoint 链上重算（#45） |
