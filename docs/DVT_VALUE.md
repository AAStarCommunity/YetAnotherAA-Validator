...
DVT
BLS 共签的核心价值一句话:**把"一把私钥说了算"的单点账户,变成"多方 + 多因子都点头才放行"的把关结构。**
它不替代 owner 的密钥,而是在 owner 之外加一道**独立、owner 自己也改不动**的关。
...
2. **节点自推 hash**:节点拿到**完整 UserOperation**，自己用
   `EntryPoint.getUserOpHash`
   算权威 hash（绑定 sender/chainId/EntryPoint），**绝不信调用方给的裸 hash**。
3. **Fail-closed
   403**:拿不出"对该 hash 的合法 owner 签名"的请求一律 403，绝不签。
4. **独立策略门**（§7）:节点按**自己的策略**（链上 PolicyRegistry 的每笔上限/收款白名单/资产限额）拒签越界 op——owner 和 CA 都改不了这套规则。
5. **带外确认门**:高额 op 在独立通道确认前**扣住不放**。
6. **强制 BLS 账户**:`approvedAlgIds=[0x01]`
   排除 ECDSA(0x02)，**不允许降级**回单签路径绕过多方。
...
- **对"密钥被盗 / 内部共谋"**（5、7、8 类）:**限损 + 抬门槛**
  —— 单点变多方多因子,越界被独立策略门和确认门挡住,把"一把钥匙清零"降级成"策略内有限损失,且需多方同时沦陷才可能扩大"。

> 核心:**没有任何单一角色（owner
> / 单节点 / 协调器 / 网络攻击者）能独自让账户动钱**。要动,必须 owner 授权 ∧ 够门限的独立节点共签 ∧ 落在 owner 自己都改不动的策略内 ∧ 高额还要带外确认。这就是它"把关"的全部意义。
