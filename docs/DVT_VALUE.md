...
## 8. 威胁模型 —— BLS 共签防住的极端场景（作恶 / 共谋 / 欺诈 / 被盗）

DVT BLS 共签的核心价值一句话:**把"一把私钥说了算"的单点账户,变成"多方 + 多因子都点头才放行"的把关结构。** 它不替代 owner 的密钥,而是在 owner 之外加一道**独立、owner 自己也改不动**的关。

### 8.1 它由几道独立的关构成（都在本仓库代码里）

1. **双因子绑定**:一笔 op 放行,既要 owner 的 ECDSA 签名（`ownerAuth`，EIP-191），又要**多个独立 DVT 节点的 BLS 共签聚合**（链上 EIP-2537 按注册公钥集合验）。缺一不可。
2. **节点自推 hash**:节点拿到**完整 UserOperation**，自己用 `EntryPoint.getUserOpHash` 算权威 hash（绑定 sender/chainId/EntryPoint），**绝不信调用方给的裸 hash**。
3. **Fail-closed 403**:拿不出"对该 hash 的合法 owner 签名"的请求一律 403，绝不签。
4. **独立策略门**（§7）:节点按**自己的策略**（链上 PolicyRegistry 的每笔上限/收款白名单/资产限额）拒签越界 op——owner 和 CA 都改不了这套规则。
5. **带外确认门**:高额 op 在独立通道确认前**扣住不放**。
6. **强制 BLS 账户**:`approvedAlgIds=[0x01]` 排除 ECDSA(0x02)，**不允许降级**回单签路径绕过多方。

### 8.2 极端场景 × 防住了什么

| # | 极端情况（作恶/共谋/欺诈/被盗） | 没有 DVT 的漏洞 | DVT BLS 如何把关 | 残留 / 前提 |
|---|---|---|---|---|
| 1 | **网络攻击者（无任何密钥）** 想让节点替某账户签 | 骗到签名就能动钱 | Fail-closed 403:无合法 `ownerAuth` 一律拒;节点自推 hash 绑定 sender | 无 |
| 2 | **跨账户 oracle 攻击**:用**自己**账户 A 的 owner 签名套**受害者** B 的 op | 信调用方 hash 就能拿 A 的签名换 B 的操作 | 节点从完整 userOp 自推 hash + 链上读 `account.owner()`，要求 `ownerAuth===owner(B)` → A 的签名对不上 → 403 | 无 |
| 3 | **单个 DVT 节点叛变/被黑** | （单签世界无此角色） | 单节点 partial sig **聚合不出有效签名**，链上门限验证过不了;且无 owner 授权不会主动签 | 需达到门限的多数仍诚实 |
| 4 | **中心协调器/聚合方欺诈** | 若能伪造就成新单点 | 它**无法让节点签未授权的东西**:每节点各自验 ownerAuth + 自推 hash + 自己策略后才签;链上按注册公钥验聚合 → 伪造聚合失败 | 协调器只搬运、不持签名权 |
| 5 | **owner 私钥被盗**（最常见的"被盗"） | 单签账户:**直接清零** | 贼有 owner 签名,但**独立策略门**拒越界（超上限/非白名单收款人）+ **带外确认门**扣住高额;策略 owner/贼都改不动 | ⚠️ Stage-1 owner-auth 门**本身不防**私钥被盗 —— 靠 Stage 2 策略门 + 确认门;**策略内的小额**仍可能被盗（把关是"限损"非"绝对挡死"） |
| 6 | **算法降级攻击**:拿 owner ECDSA 键走单签绕开多方 | 能降级则 DVT 形同虚设 | 强制 BLS 账户 `approvedAlgIds=[0x01]` 排除 0x02，合约层强制 BLS-triple 路径 | 账户须配置为 mandatory-BLS |
| 7 | **M-1 个节点共谋** | （单签世界无此概念） | 不到门限**聚合不出有效签名**;独立密钥/操作方/主机 → 共谋成本高 | 需 ≥ 门限数节点共谋且配合 |
| 8 | **owner 与部分节点共谋 / owner 键被盗 + 部分节点配合** | 单签:owner 一人就够 | 仍受**独立策略门**约束（链上 PolicyRegistry，共谋方也改不了）+ 确认门 → 越界照拒 | "owner 签名 + 够门限节点 + op 在策略内"三者同时满足才放行 —— 抬门槛 + 限损,非无限防御 |
| 9 | **TOCTOU / hash 调包**:先用合法 hash 过检再换恶意 op | 检查与签名间有窗口就能调包 | 节点**只签自己推导并已授权的那个 hash**，无调用方 hash、无中间窗口 | 无 |
| 10 | **重放**:旧的已签 op 再用一次 | 重放即重复扣款 | hash 经 `getUserOpHash` 绑定 nonce/chainId/EntryPoint;EIP-3009 nonce 链上防重放 | 无 |

### 8.3 把关逻辑总结

- **对"无密钥的外部攻击/欺诈/调包/跨账户/重放"**（2、4、9、10 类）:**绝对挡死**（fail-closed + 自推 hash + 链上验聚合）。
- **对"密钥被盗 / 内部共谋"**（5、7、8 类）:**限损 + 抬门槛** —— 单点变多方多因子,越界被独立策略门和确认门挡住,把"一把钥匙清零"降级成"策略内有限损失,且需多方同时沦陷才可能扩大"。

> 核心:**没有任何单一角色（owner / 单节点 / 协调器 / 网络攻击者）能独自让账户动钱**。要动,必须 owner 授权 ∧ 够门限的独立节点共签 ∧ 落在 owner 自己都改不动的策略内 ∧ 高额还要带外确认。这就是它"把关"的全部意义。
