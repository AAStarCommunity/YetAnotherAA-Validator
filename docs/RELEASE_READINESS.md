...
| 6   | **Watchlist should be DERIVED on-chain, not hand-curated**                   | 🟡 (new)        | auditor               | Today `AUDIT_WATCHLIST` is a static env list. The registered operators ARE on-chain (`Registry.roleMembers[ROLE_ANODE/ROLE_DVT]`, `getRoleUserCount`, reconstructable from `RoleRegistered`/`RoleExited` events). Enhance the audit to enumerate registered operators on-chain and audit all of them (`AUDIT_WATCHLIST` → optional override). Needs either a `getRoleMembers` getter from @repo:sp or an event scan.                                                                                                                                                            |
| 7   | **Only 1 of 4 audit rules implemented**                                      | 🟡 (scope)      | auditor               | ✅ `credit-over-limit` (debt>creditLimit). ❌ offline-via-gossip-heartbeat, ❌ proof-forgery, ❌ token-abuse — designed in CC-13, NOT built. Decide which to ship for prod v1.                                                                                                                                                                                                                                                                                                                                                                                                  |
