...
| 7   | **4 audit rules — building all four (CC-13 decision)**                       | 🟡 (in progress) | auditor               | ✅ `credit-over-limit`. ✅① watchlist-derive. ✅② offline **inc-1** (`feat/audit-offline-detection`): gossip heartbeat → deterministic block-anchored proof → file proposal; FILE-ONLY (armed BLS co-sign + heartbeat authentication = inc-2). → ③ token-over-issue (社区超发 mint; on-chain issuance formula @repo:sp, CC-28) → ④ proof-forgery. NOTE: the account per-tx-cap / contract-allowlist "account rules" are ALREADY enforced PROACTIVELY at sign-time via `PolicyRegistry.checkPolicy` (DVT refuses to co-sign a violating UserOp) — NOT an audit rule.                                                                                    |
