...
describe("GossipService liveness ledger (offline-audit rule ②)", () => {
  // nodeId = keccak256(pubkey) = 32 bytes = 0x + 64 hex (the shape the ledger accepts).
  const NID_X = "0x" + "1a".repeat(32);
  const NID_Y = "0x" + "2b".repeat(32);
  const NID_GHOST = "0x" + "3c".repeat(32);

  it("records lastSeen on heartbeat and returns null for a never-seen node", () => {
    const svc = buildService();
    expect(svc.getLastSeen(NID_X)).toBeNull();
    const before = Date.now();
    (svc as any).handleHeartbeatMessage({ from: NID_X } as GossipMessage);
    const seen = svc.getLastSeen(NID_X);
    expect(seen).not.toBeNull();
    expect(seen!).toBeGreaterThanOrEqual(before);
    expect(svc.getLastSeen(NID_Y)).toBeNull();
  });

  it("advances lastSeen on a later heartbeat", () => {
    const svc = buildService();
    (svc as any).handleHeartbeatMessage({ from: NID_Y } as GossipMessage);
    const first = svc.getLastSeen(NID_Y)!;
    (svc as any).handleHeartbeatMessage({ from: NID_Y } as GossipMessage);
    expect(svc.getLastSeen(NID_Y)!).toBeGreaterThanOrEqual(first);
  });
  ...
  it("ledger survives peer cleanup (records lastSeen even with no peer in the connection map)", () => {
    ...
    (svc as any).handleHeartbeatMessage({ from: NID_GHOST } as GossipMessage);
    expect((svc as any).peers.has(NID_GHOST)).toBe(false);
    expect(svc.getLastSeen(NID_GHOST)).not.toBeNull();
  });

  it("REJECTS a malformed nodeId (shape gate — anti-garbage-flood, Codex High-1)", () => {
    const svc = buildService();
    (svc as any).handleHeartbeatMessage({ from: "not-a-nodeid" } as GossipMessage);
    (svc as any).handleHeartbeatMessage({ from: "0x1234" } as GossipMessage); // too short
    (svc as any).handleHeartbeatMessage({ from: "" } as GossipMessage);
    expect(svc.getLastSeen("not-a-nodeid")).toBeNull();
    expect((svc as any).lastSeenLedger.size).toBe(0);
  });

  it("BOUNDS the ledger — a NEW nodeId past the cap is rejected, existing entries preserved (OOM guard)", () => {
    const svc = buildService();
    const cap = (GossipService as any).MAX_LIVENESS_LEDGER as number;
    // Fill to the cap with distinct valid nodeIds.
    for (let i = 0; i < cap; i++) {
      const nid = "0x" + i.toString(16).padStart(64, "0");
      (svc as any).handleHeartbeatMessage({ from: nid } as GossipMessage);
    }
    expect((svc as any).lastSeenLedger.size).toBe(cap);
    const firstNid = "0x" + (0).toString(16).padStart(64, "0");
    const before = svc.getLastSeen(firstNid);
    // One more DISTINCT nodeId → rejected (not added); the map does not grow.
    (svc as any).handleHeartbeatMessage({ from: "0x" + "ff".repeat(32) } as GossipMessage);
    expect((svc as any).lastSeenLedger.size).toBe(cap);
    expect(svc.getLastSeen("0x" + "ff".repeat(32))).toBeNull();
    // An UPDATE to an existing entry still applies (never evicted).
    (svc as any).handleHeartbeatMessage({ from: firstNid } as GossipMessage);
    expect(svc.getLastSeen(firstNid)!).toBeGreaterThanOrEqual(before!);
  });
});
